...
class UntimedModule extends MultiIOModule with MethodParent with RequireAsyncReset {
  ...
  def getChirrtl: firrtl.CircuitState = { // FIXME: this should not be part of the public API
    assert(_isElaborated, "You need to elaborate the module using UntimedModule(new ...)!")
    _chirrtl.get
  }
  def getFirrtl: firrtl.CircuitState = { // FIXME: this should not be part of the public API
    assert(_isElaborated, "You need to elaborate the module using UntimedModule(new ...)!")
    _lowfir
  }
  def getTester: treadle.TreadleTester = { // FIXME: this should not be part of the public API
    assert(_isElaborated, "You need to elaborate the module using UntimedModule(new ...)!")
    _tester
  }
